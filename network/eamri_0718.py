...
class eamri_0718(nn.Module):
    ...
    def __init__(self, 
        ...
        """
        super(eamri_0718, self).__init__()
    ...
    def forward(self, x1, y, m, sens_map=None): # (image, kspace, mask)
        ...
        e2 = self.edgeNet(x1) 
        x1 = self.fuse(x2, e2, y, m, sens_map)

        # third stage
        x2 = self.net3(x1, y, m) #(B, 2, H, W)
        e3 = self.edgeNet(x1) 
        x1 = self.fuse(x2, e3, y, m, sens_map)

        # fourth stage
        x2 = self.net4(x1, y, m) #(B, 2, H, W)
        e4 = self.edgeNet(x1) 
        x1 = self.fuse(x2, e4, y, m, sens_map)

        # fourth stage
        x2 = self.net5(x1, y, m) #(B, 2, H, W)
        e5 = self.edgeNet(x1) 
        x1 = self.fuse(x2, e5, y, m, sens_map)
