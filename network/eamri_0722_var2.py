"""
modified at 16/08/22
change edge attention to concat 
"""
...
class EAM(nn.Module):
    def __init__(self, C, C1, C2, num_heads, bias, shift=False):
        ...
        self.b1 = nn.Conv2d(C+C1, C, kernel_size=1, bias=True) # 1*1 conv
        self.dc = DC_multicoil(shift=shift)

    def forward(self, x, e, y, m, sens_map=None):
        
        x1 = torch.cat([x,e], 1) #(B, C+C1)
        xout = self.b1(x1) #(B, C, H, W)
        xout = self.dc(xout, y, m, sens_map) #(B, coils, H, W, 2)

        return xout


class eamri_0722_var2(nn.Module):
    """
    change edge attention to simply concat
    """
    def __init__(self, 
                indim=2, 
                edgeFeat=16, 
        ...
        """
        super(eamri_0722_var2, self).__init__()
