...
class eamri_0828(nn.Module):
    ...
    def __init__(self, 
        ...
        """
        super(eamri_0828, self).__init__()
        ...
        self.net5 = rdn_convBlock(inChannel=indim, midChannel=fNums[5], recursiveTime=num_iters[5], shift=shift)
        ...
        self.fuse5 = EAM(indim, 1, attdim, num_head, bias=False)

        self.comb1 = weightedAverageTerm(shift=shift)
        self.comb2 = weightedAverageTerm(shift=shift)
        self.comb3 = weightedAverageTerm(shift=shift)
        self.comb4 = weightedAverageTerm(shift=shift)
        self.comb5 = weightedAverageTerm(shift=shift)
    ...
    def forward(self, x1, y, m): # (image, kspace, mask)
        ...
        # image head 
        x_cnn = self.imHead(x1, y, m, sens_map) #(B, 2, H, W)
        
        # first stage
        x_de = self.net1(x_cnn, y, m, sens_map) #(B, 2, H, W)
        e2 = self.edgeNet(x_cnn) #(B, 1, H, W)
        x_eam = self.fuse1(x_cnn, e2) 
        x_cnn = self.comb1(x_de, x_eam, y, m, sens_map)

        # second stage
        x_de = self.net2(x_cnn, y, m, sens_map) #(B, 2, H, W)
        e3 = self.edgeNet(x_cnn) 
        x_eam = self.fuse2(x_cnn, e3)
        x_cnn = self.comb2(x_de, x_eam, y, m, sens_map)

        # third stage
        x_de = self.net3(x_cnn, y, m, sens_map) #(B, 2, H, W)
        e4 = self.edgeNet(x_cnn) 
        x_eam= self.fuse3(x_cnn, e4) 
        x_cnn = self.comb3(x_de, x_eam, y, m, sens_map)

        # fourth stage
        x_de = self.net4(x_cnn, y, m, sens_map) #(B, 2, H, W)
        e5 = self.edgeNet(x_cnn) 
        x_eam = self.fuse4(x_cnn, e5) 
        x_cnn = self.comb4(x_de, x_eam, y, m, sens_map)

        # fifth stage
        x_de = self.net5(x_cnn, y, m, sens_map) #(B, 2, H, W)
        e6 = self.edgeNet(x_cnn) 
        x_eam = self.fuse5(x_cnn, e5) 
        x_cnn = self.comb5(x_de, x_eam, y, m, sens_map)

        return [e2,e3,e4,e5,e6,x_cnn]
