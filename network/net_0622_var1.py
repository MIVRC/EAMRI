...
class imExtractor(nn.Module):
    ...
    def __init__(self, indim=2, 
        ...
        super(imExtractor, self).__init__()
        layers = []
        for _ in range(n_DAM):
            layers.append(DAM2(indim, fNum, 2, 1, growthRate, layer, dilate, activation, useOri, transition))
...
class net_0622_var1(nn.Module):
    """
    12 DAM + transformer block
    """
    def __init__(self, img_size=320, indim=2, edgeFeat=16, outdim=12, num_head=4, n_DAM=1, isFastmri=False): 
        """
        input:
        ...
        """
        super(net_0622_var1, self).__init__()
        
        # image module
        self.net1 = imExtractor(isFastmri=isFastmri, n_DAM=n_DAM)
        self.net2 = imExtractor(isFastmri=isFastmri, n_DAM=n_DAM)
        self.net3 = imExtractor(isFastmri=isFastmri, n_DAM=n_DAM)
        self.net4 = imExtractor(isFastmri=isFastmri, n_DAM=n_DAM)
