...
class EEM(nn.Module):
    def __init__(self, C, C1, C2, num_heads, bias):
        ...
        self.norm = LayerNorm(C, 'Bias_Free')
        self.mlp = Mlp(C, C//2)
    ...
    def forward(self, x, e, sens_map=None):
        ...
        # mlp
        #out1 = self.mlp(self.norm(out).permute(0,2,3,1).contiguous()) #(B, H, W, C)
        #out = out + out1.permute(0,3,1,2) #(B, C, H, W)
        
        return out.contiguous() #(B, C, H, W)
...
class eamri_0719_debug4(nn.Module):
    """
    ...
    def __init__(self, 
        ...
        """
        super(eamri_0719_debug4, self).__init__()
