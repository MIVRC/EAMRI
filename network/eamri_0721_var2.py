...
class NormUnet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.complex_to_chan_dim(x) #(96, 2, 218, 170)
...
class rdn_convBlock(nn.Module):
    def __init__(self, convNum = 3, recursiveTime = 3, inChannel = 2, midChannel=16, isFastmri=False, isMulticoil=False):
        ...
        self.dilateBlock = dilatedConvBlock(convNum, midChannel)
        self.conv2 = nn.Conv2d(midChannel,inChannel,3,padding = 1)
        self.dc = dataConsistencyLayer_multi(isFastmri=isFastmri, isMulticoil=isMulticoil) 
    ...
    def forward(self, x1, y, m, sens_map):
        ...
        # dc
        x5= T.expand_operator(x5.permute(0,2,3,1), sens_map, dim=1) #(B, coils, H, W, 2)
        x5 = self.dc(x5, y, m) #(B, coils, H, W, 2)
        x5 = T.reduce_operator(x5, sens_map, dim=1) #(B, H, W, 2)

        return x5.permute(0,3,1,2).contiguous()
...
class attBlock(nn.Module):
    ...
    def forward(self, x, e, y, m, sens_map=None):
        ...
        xout = self.b1(x, e) #(B, H, W, 2)
        xout = T.expand_operator(xout, sens_map, dim=1) #(B, coils, H, W, 2)
        xout = self.dc(xout, y, m) #(B, coils, H, W, 2)
        xout = T.reduce_operator(xout, sens_map, dim=1) #(B, H, W, 2)

        return xout.permute(0,3,1,2).contiguous()


class eamri_0721_var2(nn.Module):
    """
    ...
    def __init__(self, 
        ...
        """
        super(eamri_0721_var2, self).__init__()
    ...
    def forward(self, x1, y, m, sens_map=None): # (image, kspace, mask)
        ...
        # reduce 
        B, coils, H, W= x1.shape
        x1 = x1.reshape(B, -1, H, W, 2)
        x1 = T.reduce_operator(x1, sens_map, dim=1) #(B, H, W, 2)
        x1 = x1.permute(0,3,1,2) #(B,2,H,W)
        
        # image head 
        x1 = self.net1(x1, y, m, sens_map) #(B, 2, H, W)
