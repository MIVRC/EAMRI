...
class Edge_Net(nn.Module):
    def __init__(self, indim, hiddim, conv=default_conv, n_MSRB=3):
        ...
        modules_head = [conv(indim//2, hiddim, kernel_size)] #3*3 conv
        # body
    ...
    def forward(self, x):
        ...
        B, C, H, W = x.shape
        x_abs = x.reshape(B, -1, H, W, 2) #(B, C//2, H, W, 2)
        x_abs = (x_abs ** 2).sum(dim=-1) #(B, C//2, H, W)
        x = self.norm(x_abs) #(B, C//2, H, W)
...
class eamri_0715_var1(nn.Module):
    """
    ...
    def __init__(self, 
        ...
        """
        super(eamri_0715_var1, self).__init__()
    ...
    def forward(self, x1, y, m, sens_map=None): # (image, kspace, mask)
        ...
        # second stage
        x2 = checkpoint.checkpoint(self.net2, x1, y, m) #(B, 2, H, W)
        e2 = self.edgeNet(x1) 
        x1 = self.fuse(x2, e2, y, m)

        # third stage
        x2 = checkpoint.checkpoint(self.net3, x1, y, m) #(B, 2, H, W)
        e3 = self.edgeNet(x1) 
        x1 = self.fuse(x2, e3, y, m)

        # fourth stage
        x2 = checkpoint.checkpoint(self.net4, x1, y, m) #(B, 2, H, W)
        e4 = self.edgeNet(x1) 
