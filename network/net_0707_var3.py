...
class net_0707_var3(nn.Module):
    """
    12 DAM + transformer block
    """
    def __init__(self, img_size=320, indim=2, edgeFeat=16, attdim=4, num_head=4, n_DAMs=[1,1,3,1], layers=[4,4,4,4], num_iters=[3,3,1,3], isFastmri=False): 
        """
        input:
        ...
        """
        super(net_0707_var3, self).__init__()
        
        # image module
        self.net1 = convBlock(isFastmri=isFastmri, n_DAM=n_DAMs[0], layer=layers[0], num_iter=num_iters[0])
        self.net2 = convBlock(isFastmri=isFastmri, n_DAM=n_DAMs[1], layer=layers[1], num_iter=num_iters[1])
        self.net3 = convBlock(isFastmri=isFastmri, n_DAM=n_DAMs[2], layer=layers[2], num_iter=num_iters[2])
        self.net4 = convBlock(isFastmri=isFastmri, n_DAM=n_DAMs[3], layer=layers[3], num_iter=num_iters[3])
