...
class ASIM(nn.Module):
    def __init__(self, in_feat=1, mid_feat=96, M=2, r=16 ,stride=1, L=16):
        ...
        for i in range(M): # for each branch
            self.split_convs.append(
                    nn.Sequential(
                        nn.Conv2d(in_feat, mid_feat, kernel_size=1, bias=True), # use 1*1 kernel
        ...
        self.decoder = nn.Conv2d(mid_feat,in_feat,1)
    ...
    def __init__(self, dim, patch_size, input_resolution, depth, num_heads, window_size,
        ...
        for i in range(depth):
            if i % 3 == 0:
                ...
                blocks.append(EdgeAttBlock(img_size=input_resolution, patch_size=patch_size, dim=dim))
...
class RPTL(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False,
                 img_size=224, patch_size=4, resi_connection='1conv',shift=False):
