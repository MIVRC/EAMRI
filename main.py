    ...
    with torch.no_grad():
        for iter, data in tqdm(enumerate(data_loader)):
            if iter % args.jump == 0: # every 10 iter
                ...
                res_np = 5 * (np.abs(gt_np - pred_np) / gt_np.max()) # scaling for better visualization
...
def create_arg_parser_fastmri():
    ...
    parser.add_argument('--valid_root', type=str, help='path to store the test data')
    ...
    parser.add_argument('--data_parallel', action='store_true',default=True, help='If set, use multiple GPUs using data parallelism')
